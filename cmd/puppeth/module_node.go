...
{{end}}
RUN \
  echo 'ghpmc --cache 512 init /genesis.json' > ghpmc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.happymadedium/keystore/ && cp /signer.json /root/.happymadedium/keystore/' >> ghpmc.sh && \{{end}}
	echo $'exec ghpmc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> ghpmc.sh

ENTRYPOINT ["/bin/sh", "ghpmc.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.happymadedium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.hpmchash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.happymadedium"],
		ethashdir:  infos.volumes["/root/.hpmchash"],
		port:       port,
